...
import { CLIENT_ID, DISCORD_TOKEN, HOME_GUILD_ID } from './environment';
...
export async function deployCommands(commands: Collection<string, BotCommand>) {
    ...
    await request
        .put(Routes.applicationGuildCommands(CLIENT_ID, HOME_GUILD_ID), {
            body: commandsJSON,
        })
        .then(() => log('Deployed commands to guild'))
        .catch(log);
}
